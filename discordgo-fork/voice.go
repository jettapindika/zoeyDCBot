...
func (v *VoiceConnection) opusSender(udpConn *net.UDPConn, close <-chan struct{}, opus <-chan []byte, rate, size int) {
	...
	for i := uint32(0); ; i++ {
		...
		// DAVE E2EE encryption (before transport encryption)
		v.RLock()
		...
		if daveActive {
			...
			if err != nil {
				...
				if i < 3 {
					v.log(LogDebug, "opusSender[%d]: DAVE encrypted %d -> %d bytes", i, len(recvbuf), len(encrypted))
				}
				recvbuf = encrypted
			}
		}
		...
		// Discord expects the 4-byte nonce counter in NetworkEndian (BigEndian),
		// placed at the start of the full AEAD nonce (remaining bytes zero).
		// Reference: songbird (serenity-rs) uses NetworkEndian for all *_rtpsize modes.
...
func (v *VoiceConnection) opusReceiver(udpConn *net.UDPConn, close <-chan struct{}, c chan *Packet) {
	...
	for {
		...
		// Discord sends the 4-byte nonce counter in NetworkEndian (BigEndian).
		binary.BigEndian.PutUint32(nonce[:4], binary.BigEndian.Uint32(nonceCounter))
...
func (v *VoiceConnection) handleDAVEBinary(message []byte) {
	...
	case 30:
		...
		v.log(LogInformational, "DAVE DeriveSenderKey OK")

		dave.HandlePrepareTransition(transitionID, 1)
		if err := dave.HandleExecuteTransition(transitionID); err != nil {
			v.log(LogError, "DAVE execute transition after welcome failed: %s", err)
			return
		}
		v.log(LogInformational, "DAVE encryption activated, IsActive=%v", dave.IsActive())
